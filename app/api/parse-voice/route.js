...
- date : date + heure de l'événement, format "YYYY-MM-DDTHH:MM". Ne renseigne le champ "date" QUE si un JOUR ou une DATE est explicitement mentionné (aujourd'hui, demain, samedi, ce week-end, le 5 juin...). Résous les dates relatives depuis la date du jour fournie. Si l'organisateur ne donne QU'UNE HEURE sans jour (ex : "20h", "à 18h30") et qu'aucune date n'existe déjà dans current.date, N'INCLUS PAS le champ "date" du tout — ne fabrique JAMAIS la date du jour par défaut, ne déduis jamais "aujourd'hui" à partir d'une simple heure. Si une date avec un jour existe déjà dans current.date et que l'utilisateur donne une heure, fusionne (garde le jour, remplace l'heure). Si un jour est donné sans heure, mets l'heure à "00:00".
- nb_participants : entier (nombre de personnes).
- location : chaîne (le lieu : "chez moi", "chez Thomas", une adresse, un parking...).
- organizer_name : le prénom de l'organisateur UNIQUEMENT s'il se présente explicitement (ex : "je m'appelle X", "moi c'est X", "c'est moi qui organise, X", "organisé par X"). ATTENTION : "chez X" (ex : "chez Adam", "chez Thomas") désigne le LIEU — mets-le dans "location" et NE renseigne PAS organizer_name à partir de ça. En cas de doute, n'extrais PAS organizer_name (mieux vaut le demander).
